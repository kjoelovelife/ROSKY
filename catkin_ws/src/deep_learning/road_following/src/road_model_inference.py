...
class Inference_Model_Node(object):
    def __init__(self):
        ...
        ## read information
        self.recording = self.read_param_from_file(package=self.package, folder="model", file_name="recording.yaml")
        self.inference_parameter = self.read_param_from_file(package=self.package, folder="param", file_name="inference_model.yaml")
        ...
        if self.check_model_exist(self.model_name):
            ...
            self.process_img_mean  = torch.Tensor(self.inference_parameter["preprocess_mean"]).cuda().half()
            self.process_img_stdev = torch.Tensor(self.inference_parameter["preprocess_stdev"]).cuda().half()
    ...
    def check_model_exist(self, name):
        if not name in self.recording.keys():
            rospy.logwarn("[{}] {} does not exist in folder [model]. Please check your model_pth in [{}].".format(self.node_name, name, self.getFilePath(package=self.package, folder="model", file_name="inference_model.yaml") ))
    ...
    def load_model(self, model="resnet18", param_pretrained=False, kind_of_classifier=2):
        ...
        if model in model_list:
            ...
            model_pth = self.getFilePath(package=self.package, folder="model", file_name=self.model_name)
            self.model.load_state_dict(torch.load(model_pth))
            interval = rospy.get_time() - start_time
            rospy.loginfo("[{}] Loading model done! Use {:.2f} seconds.".format(self.node_name, interval))
            return self.model
        else:
