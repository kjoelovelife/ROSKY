...
class Inference_To_Reaction(object):
    def __init__(self):
        ...
        _start = rospy.wait_for_message("~road_following_inference", road_following_inference_msg)
        _start = rospy.wait_for_message("~img_recognition_inference", img_recognition_inference_msg)
        ...
        self.want_labels = ["PCB", "blocked", "free", "paper", ]
    ...
    def inference_analyze(self, msg_road_following, msg_img_recognition):
        ...
        if self.confidence[recognition] > self.confidence_threshold: 
            car_cmd_msg.v = {
                ...
                "PCB": 0,
                "paper": 0,
            }.get(recognition, self.pid_parameter["speed_gain"]) 
        else:
            pass
