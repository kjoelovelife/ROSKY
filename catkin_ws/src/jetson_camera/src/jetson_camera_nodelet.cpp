...
private:
	static std::string gstreamer_pipeline (int capture_device, int capture_width, int capture_height, int display_width, int display_height, int framerate, int flip_method) {
	...
	int cap_device_, cap_width_, cap_height_;
	...
	void onInit() override
	{
		...
		image_transport::ImageTransport it_priv(nh_priv_);
                ...
                cap_device_ = nh_priv_.param<int>("cap_device", 0);
		...
		NODELET_INFO("jetson_camera: Starting node with the following parameters:\n"\
		             "capture device: %d, capture width: %d, capture height: %d\n"\
		             "published width: %d, published height: %d\n"\
		             "requested framerate: %d, flip method: %d\n",
		             cap_device_,cap_width_, cap_height_, width_, height_, framerate_, flip_method_);
	...
	void captureFunc()
	{
		std::string pipeline = gstreamer_pipeline(cap_device_, cap_width_, cap_height_, width_, height_, framerate_, flip_method_);
