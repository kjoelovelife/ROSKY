...
class CamInfoReader(object):
    def __init__(self):
        ...
        # Load parameters
        self.pub_freq       = self.setupParam("~pub_freq",1)
        ...
        self.cali_file = rospack.get_path('rosky_base') + "/config/" + self.config + "/calibration/camera_intrinsic/" +  self.cali_file_name + ".yaml" 
        self.camera_info_msg = None

        # Load calibration yaml file
        if not os.path.isfile(self.cali_file):
            rospy.logwarn("[%s] Can't find calibration file: %s.\nUsing default calibration instead." %(self.node_name,self.cali_file))
            self.cali_file = rospack.get_path('rosky_base') + "/config/" + self.config + "/calibration/camera_intrinsic/default.yaml" 
        ...
        self.sub_img_compressed = rospy.Subscriber("~compressed_image",img_type,self.cbCompressedImage,queue_size=self.pub_freq)
